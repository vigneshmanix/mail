...
<title>DRAFT-<?php echo $_SESSION['usr'];?></title>
<?php
include("connet.php");
if($_SESSION['conf']==0)
  ...
  <tr>
    ...
    <td width="90%" align="left" valign="top"><form id="form1" name="form1" method="post" action="">
      <table width="100%" border="0">
		...
		<?php
			$to1=$_SESSION['usr'];
			$qry="select * from tblmail2 where from1='$to1' and DS=1";
		...
		{$qry="update tblmail2 set SS=1 where id='$ch' ";
		mysql_query($qry);
		$qry="update tblmail2 set DS=0 where id='$ch' ";
		mysql_query($qry);
        ...
        <label>
        <input type="submit" name="s6" id="s6" value="Send Selected" />
        </label>
    </form>
    </td>
  </tr>
