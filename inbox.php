...
<title>INBOX-<?php echo $_SESSION['usr'];?></title>
<?php
  ...
  <tr>
    ...
    <td width="90%" align="left" valign="top"><form id="form1" name="form1" method="post" action="">
      <table width="100%" border="0">
		...
		<?php
			$to1=$_SESSION['usr'];
			$qry="select * from tblmail2 where to1='$to1' and SS=1 ";
		...
		{$qry="update tblmail2 set TS=1 where id='$ch' ";
		mysql_query($qry);
		$qry="update tblmail2 set SS=0 where id='$ch' ";
		mysql_query($qry);
        ...
        <label>
        <input type="submit" name="s6" id="s6" value="Delete Selected" />
        </label>
    </form>
    </td>
  </tr>
